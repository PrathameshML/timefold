...
function getPayload(employees, timeLimit) {
    return {
        ...
        "unimproved_time_limit_seconds": timeLimit, // Ensure unimproved limit doesn't kill it early
...
async function runBenchmark() {
    ...
    // As per user request: Run up to 600 seconds to prove the plateau
    const times = [300, 450, 600];
    
    let report = "| Time | Hard | Medium | Soft | Coverage | Avg Wage | Avg Rating |\n|---|---|---|---|---|---|---|\n";

    for (let t of times) {
        console.log(`Running benchmark for ${t}s...`);
        const payload = getPayload(employees, t);
        ...
        if (res) {
            ...
            const row = `| ${t}s | ${score.hard} | ${score.medium} | ${score.soft} | ${res.new_assignments_made}/${res.total_possible_assignments} | $${avgWage}/hr | 3.0 |`;
            console.log(row);
            report += row + "\n";
            
            fs.writeFileSync(`explanation_${t}s.txt`, exp);
        } else {
             console.log(`Failed for ${t}s`);
        }
    }
    
    fs.writeFileSync('benchmark_report.md', report);
    console.log("Benchmark complete. Wrote benchmark_report.md");
}
