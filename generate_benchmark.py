...
# 4 shifts, each needs ~35 workers per day = 140 total slots/day
shifts = [
    {
...
# Total slots per day: 35 + 35 + 26 + 26 = 122 slots/day * 7 days = 854 total slots
# Total employees: 200 (40 per role)
# Slack: 200 employees, 122 needed per day, but each employee can only work 1 shift/day
...
with open("c:/Users/user/Downloads/timefold/timefold/benchmark_payload.json", "w") as f:
    json.dump(payload, f, indent=2)

# Summary
total_per_day = sum(sum(r["max_workers"] for r in s["roles"]) for s in shifts)
print(f"Employees: {len(employees)}")
print(f"Shifts: {len(shifts)}")
print(f"Days: 7")
print(f"Slots per day: {total_per_day}")
print(f"Total slots: {total_per_day * 7}")
print(f"Roles per employee: {dict((r, sum(1 for e in employees if e['role'] == r)) for r in roles)}")
print(f"Payload written to benchmark_payload.json")
