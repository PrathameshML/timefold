function Generate-Employees {
    ...
    for ($i=1; $i -le $Count; $i++) {
        $emps += @{ employee_id = "EMP-CACHE-$i"; name = "Cache Test $i"; rate = 15.0; role = "Helper"; rating = 3 }
    }
    return $emps
}
...
Write-Host "Run 1 (Cost): $($response1.new_assignments_made)"
...
Write-Host "Run 2 (Quality): $($response2.new_assignments_made)"
