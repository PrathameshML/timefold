...
function Test-Api {
    ...
    $start = Get-Date
    try {
        $response = Invoke-RestMethod -Uri "http://localhost:8083/shifts/assign-v3" -Method Post -Body $json -ContentType "application/json" -TimeoutSec 600
        $end = Get-Date
...
# Massive Scale: 500 Employees, 7 Days. Need 50 Workers per day (3500 entities total).
...
$payload180s = @{
    ...
    unimproved_time_limit_seconds = 45
    roles = @( @{ role_name = "Helper"; rating = 1; max_workers = 50 } )
    existing_users = $employees500
}
Test-Api -Name "500 Employees, 7 Days (Cost, 180s limit, Fresh Dates)" -Payload $payload180s
...
Test-Api -Name "500 Employees, 7 Days (Cost, 240s limit, Fresh Dates)" -Payload $payload240s
...
$payload300s = @{
    ...
    unimproved_time_limit_seconds = 75
    roles = @( @{ role_name = "Helper"; rating = 1; max_workers = 50 } )
    existing_users = $employees500
}
Test-Api -Name "500 Employees, 7 Days (Cost, 300s limit, Fresh Dates)" -Payload $payload300s
