...
public class ShiftResource {
    ...
    public Response assignShiftV2(Map<String, Object> input) {
        ...
        try {
            Map<String, Object> result = solverService.solveShiftV2(input);
            if ("error".equals(result.get("status"))) {
                int errorCode = result.containsKey("error_code") ? ((Number) result.get("error_code")).intValue() : Response.Status.BAD_REQUEST.getStatusCode();
                return Response.status(errorCode).entity(result).build();
            }
            return Response.ok(result).build();
        } catch (Exception e) {
            LOG.error("Failed to solve shift (V2)", e);
            return Response.serverError()
                    .entity(Map.of("status", "error", "message", e.getMessage()))
                    .build();
        }
    ...
    public Response batchAssignShiftsV2(Map<String, Object> input) {
        ...
        LOG.info("Received V2 batch assignment request with " + batchRequests.size() + " shifts (with availability support)");
        List<Map<String, Object>> results = new ArrayList<>();
        int totalAssignments = 0;
        int totalSkipped = 0;
        double totalSolverTime = 0.0;
        ...
        for (int i = 0; i < batchRequests.size(); i++) {
            Map<String, Object> request = batchRequests.get(i);
            try {
                Map<String, Object> result = solverService.solveShiftV2(request);
                result.put("shift_index", i);
                results.add(result);
                ...
                LOG.error("Failed to solve shift in V2 batch", e);
                failedShifts++;
                Map<String, Object> errorResult = new HashMap<>();
                errorResult.put("status", "error");
                errorResult.put("message", e.getMessage());
